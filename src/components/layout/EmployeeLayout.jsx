...
import {
  LayoutDashboard, MapPin, Users, Receipt, User,
  LogOut, Bell, Menu, X, ChevronRight, Zap
} from 'lucide-react';
...
export default function EmployeeLayout({ children }) {
  ...
  return (
    <div className="min-h-screen mesh-bg flex">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 h-full w-64 glass border-r border-white/10 z-50 flex flex-col transition-transform duration-300
        ...
        <div className="p-4 border-b border-white/10">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-white/40 text-xs truncate">{user?.employeeId}</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar */}
        <header className="sticky top-0 z-30 glass border-b border-white/10 px-4 lg:px-6 py-3 flex items-center justify-between">
          <button className="lg:hidden p-2 rounded-lg hover:bg-white/10" onClick={() => setSidebarOpen(true)}>
            <Menu className="w-5 h-5 text-white" />
          </button>
          <div className="hidden lg:block">
            <h1 className="text-white font-semibold text-sm">Good {getGreeting()}, {user?.name?.split(' ')[0]} 👋</h1>
            <p className="text-white/40 text-xs">{new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
          </div>
      ...
      {/* Bottom nav (mobile) */}
      <nav className="fixed bottom-0 left-0 right-0 lg:hidden z-30 glass border-t border-white/10 px-2 py-2 flex items-center justify-around">
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to}
            className={({ isActive }) =>
              `flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all duration-200 ${
                isActive ? 'text-primary-400 bg-primary-600/15' : 'text-white/40 hover:text-white/70'
              }`}>
